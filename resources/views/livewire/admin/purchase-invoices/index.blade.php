<div>
    ...
    <div class="bg-slate-800 rounded-2xl border border-slate-700/50 shadow-xl overflow-hidden" style="box-shadow: 0 10px 30px rgba(0, 0, 0, 0.3);">
        <div class="p-6 border-b border-slate-700/50 flex flex-col sm:flex-row justify-between items-center gap-4 bg-slate-800">
            ...
            <a href="{{ route('admin.purchase-invoices.create') }}" class="w-full sm:w-auto text-white bg-blue-600 hover:bg-blue-700 px-5 py-2.5 rounded-xl transition-all shadow-lg">إنشاء فاتورة</a>
        </div>
            ...
            <tbody class="divide-y divide-slate-700/50">
                @forelse($invoices as $invoice)
                    <tr class="hover:bg-slate-700/30 transition-colors">
                        ...
                        <td class="px-6 py-4 text-gray-400">{{ $invoice->company->name ?? '-' }}</td>
                        ...
                        <td class="px-6 py-4 flex gap-3">
                            <a href="{{ route('admin.purchase-invoices.view', $invoice->id) }}" class="text-blue-400 hover:text-blue-300 transition-colors">عرض</a>
                            <a href="{{ route('admin.purchase-invoices.edit', $invoice->id) }}" class="text-yellow-400 hover:text-yellow-300 transition-colors">تعديل</a>
